...
import {
	isLocationWithWaypoint,
	LocationWithWaypointSpecifier,
} from "@auto/strategy/types";
...
apiEndpoint = process.env.API_ENDPOINT.trim().replace(/\/$/, "");
...
async function loadObjectives() {
	...
	const executingObjectives = ships
		.map((s) => s.objective)
		.filter((g) => !!g) as string[];
	executingObjectives.push(
		...(ships.map((s) => s.nextObjective).filter((g) => !!g) as string[]),
	);
...
const init = async () => {
	...
	if (!account) {
		throw new Error(
			"You need to register the account specified in .env for the agent first.",
		);
	}
	...
	const serverStatus = await axios.get<StatusResponse>(`${apiEndpoint}/`);
	...
	const { executingObjectives, executorsAssignedToObjective } =
		await loadObjectives();

	const orchestrator = new Orchestrator<Ship, Task, Objective>(
		executingObjectives,
		executorsAssignedToObjective,
		{
			...
			getDistance: (ship, objective) => {
				...
				if (
					ship.currentSystemSymbol === objective.startingLocation.system.symbol
				) {
					return objective.startingLocation.waypoint
						? getDistance(
								ship.currentWaypoint,
								objective.startingLocation.waypoint,
							)
						: 0;
				}
				...
				return (
					800 +
					getDistance(ship.currentSystem, objective.startingLocation.system)
				);
			},
			getTravelTime: (ship, from, to) => {
				...
				if (
					validateEngineSpeed(ship.engineSpeed) &&
					isLocationWithWaypoint(from) &&
					isLocationWithWaypoint(to)
				) {
					return TravelTimeCache.calculate(
						ship.engineSpeed,
						ship.maxFuel,
						from,
						to,
			...
			objectiveValid: (ship, objective) => {
				const isAtOwnLocation =
					objective.startingLocation === "self" ||
					objective.startingLocation.waypoint?.symbol ===
	...
	fastify.post("/cancel-objective", async (request, reply) => {
		const { shipSymbol, objective } = request.body as {
			shipSymbol: string;
			objective: string;
		};
