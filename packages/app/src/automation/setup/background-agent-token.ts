...
export const getBackgroundAgentToken = async (server?: Server) => {
	...
	if (
		...
		console.log(
			`Registering agent ${environmentVariables.agentName} with faction ${environmentVariables.agentFaction} on server.`,
		);
			...
			.register(
				{
					...
					faction:
						environmentVariables.agentFaction as RegisterRequest["faction"],
				},
				{
					headers: {
