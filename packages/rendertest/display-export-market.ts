...
export function displayExportMarkets(
	systemSymbol: string,
	waypointSymbol: string,
	tradeGoodSymbols: string[],
) {
	const selectedWaypointData = Registry.waypointData[waypointSymbol];
	const selectedWaypoint = Registry.waypoints[waypointSymbol];

	Registry.waypointsForSystem[systemSymbol]
		.filter((wp) =>
			wp.tradeGoods?.some(
				(tg) =>
					tradeGoodSymbols.includes(tg.tradeGoodSymbol) && tg.kind === "EXPORT",
			),
		)
		.forEach((exportWaypointData) => {
			...
			marketRouteNew.rotation = Math.atan2(
				selectedWaypoint.y - exportWaypoint.y,
				selectedWaypoint.x - exportWaypoint.x,
			);
			marketRouteNew.scale = { x: 1, y: 1 };
			// make long enough to reach to the next waypoint
			marketRouteNew.width = Math.sqrt(
				Math.pow(selectedWaypoint.x - exportWaypoint.x, 2) +
					Math.pow(selectedWaypoint.y - exportWaypoint.y, 2),
			);
			marketRouteNew.height = 16;
			marketRouteNew.play();
			marketRoutes.push(marketRouteNew);

			const exports = exportWaypointData.tradeGoods?.filter(
				(tg) =>
					tradeGoodSymbols.includes(tg.tradeGoodSymbol) && tg.kind === "EXPORT",
			);
			if (exports) {
				exports.forEach((tg, idx) => {
					console.log(
						`export ${tg.tradeGoodSymbol} from ${exportWaypointData.symbol}`,
					);
					const sprite = new Sprite({
						texture:
							loadedAssets.spritesheet.textures[
								"public/textures/icons/GOOD_" + tg.tradeGoodSymbol + ".png"
							],
					});
					sprite.zIndex = -1;
...
export function displayImportMarkets(
	systemSymbol: string,
	waypointSymbol: string,
	tradeGoodSymbols: string[],
) {
	...
	Registry.waypointsForSystem[systemSymbol]
		.filter((wp) =>
			wp.tradeGoods?.some(
				(tg) =>
					tradeGoodSymbols.includes(tg.tradeGoodSymbol) && tg.kind === "IMPORT",
			),
		)
		.forEach((importWaypointData) => {
			...
			marketRouteNew.rotation = Math.atan2(
				importWaypoint.y - selectedWaypoint.y,
				importWaypoint.x - selectedWaypoint.x,
			);

			marketRouteNew.scale = { x: 1, y: 1 };
			// make long enough to reach to the next waypoint
			marketRouteNew.width = Math.sqrt(
				Math.pow(selectedWaypoint.x - importWaypoint.x, 2) +
					Math.pow(selectedWaypoint.y - importWaypoint.y, 2),
			);
			marketRouteNew.height = 16;
			marketRouteNew.play();
			marketRoutes.push(marketRouteNew);

			const imports = importWaypointData.tradeGoods?.filter(
				(tg) =>
					tradeGoodSymbols.includes(tg.tradeGoodSymbol) && tg.kind === "IMPORT",
			);
			if (imports) {
				imports.forEach((tg, idx) => {
					console.log(
						`import ${tg.tradeGoodSymbol} from ${importWaypointData.symbol}`,
					);
					const sprite = new Sprite({
						texture:
							loadedAssets.spritesheet.textures[
